...
export function TreasuryChart({
    ...
    const [chartMode, setChartMode] = useState<"treasury" | "followers" | "power">("treasury");
    ...
    useEffect(() => {
        setHistory((prev) => {
            ...
            if (last && now - last.timestamp < 4000 && last.treasury === treasuryValue) {
                return prev;
            }
            const next = [
                ...prev,
    ...
    const treasuryTrend = history.length >= 2
        ? history[history.length - 1].treasury - history[Math.max(0, history.length - 6)].treasury
        : 0;
    const followerTrend = history.length >= 2
        ? history[history.length - 1].followers - history[Math.max(0, history.length - 6)].followers
        : 0;

    // Extract data for active chart mode
    const chartData = history.map((s) =>
        chartMode === "treasury" ? s.treasury : chartMode === "followers" ? s.followers : s.power
    );
                ...
                <div className="mb-4 p-3 bg-gray-900/30 rounded-lg">
                    ...
                    <div className="flex gap-2 mb-2">
                        {(["treasury", "followers", "power"] as const).map((mode) => (
                            ...
                            >
                                {mode === "treasury" ? "💰 Treasury" : mode === "followers" ? "👥 Followers" : "⚡ Power"}
                            </button>
                        ))}
                    </div>
                    <MiniLineChart
                    ...
                    <div className="flex gap-4 mt-2">
                        <div className="text-[10px]">
                            ...
                            <span className={treasuryTrend >= 0 ? "text-green-400" : "text-red-400"}>
                                {treasuryTrend >= 0 ? "▲" : "▼"} {Math.abs(treasuryTrend).toFixed(2)} MON
                            </span>
                        </div>
                        <div className="text-[10px]">
                            <span className="text-gray-500">Followers </span>
                            <span className={followerTrend >= 0 ? "text-green-400" : "text-red-400"}>
                ...
                {bars.map((bar) => (
                    <div key={bar.label} className="flex-1 flex flex-col items-center gap-1">
                        <div className="text-xs font-mono text-gray-400">
                            {bar.value}
                            {bar.unit && <span className="text-gray-600 ml-0.5">{bar.unit}</span>}
                        </div>
                        ...
                        <div className="text-[10px] text-gray-500 text-center">{bar.label}</div>
                    </div>
                ))}
            </div>
