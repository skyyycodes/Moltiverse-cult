          ...
          {offers.map((offer) => (
            ...
            >
              <div className="flex items-center justify-between">
                ...
                <span
                  className={`text-[10px] uppercase font-bold ${statusColor(
                    offer.status,
                  )}`}
                >
                  {offer.status}
              ...
              <div className="text-xs text-white/40">
                Amount:{" "}
                <span className="text-amber-300">{offer.amount} $CULT</span>
                {" · "}
                Target:{" "}
                <span className="text-purple-400">
                  {offer.target_cult_name}
                </span>
              </div>
              <div className="text-[10px] text-white/25">
                ...
                {offer.accepted_at && (
                  <>
                    {" "}
                    · Accepted{" "}
                    {new Date(offer.accepted_at).toLocaleTimeString()}
                  </>
                )}
              </div>
...
/* ── Token Transfer Panel (On-Chain) ──────────────────────────── */

function TokenTransferPanel({
  cults,
  onAction,
}: {
  cults: Cult[];
  ...
  return (
    <Card title="⚡ On-Chain Token Transfer">
      <p className="text-[10px] text-white/30 mb-3">
        Real $CULT ERC-20 transfer on Monad. Auto-faucets tokens if sender
        balance is low.
      </p>
      <div className="flex gap-2 flex-wrap items-end">
        <div>
          <label className="text-[10px] text-white/30 block mb-1">From</label>
          <select
            value={from}
            onChange={(e) =>
        ...
        <div>
          <label className="text-[10px] text-white/30 block mb-1">To</label>
          <select
            value={to}
            onChange={(e) =>
        ...
        <ActionButton
          ...
          onClick={async () => {
            ...
            setLastTx(null);
            try {
              const result = await adminApi.tokenTransfer(
                from as number,
                to as number,
                amount,
              );
              ...
              onAction("token_transfer", `tx: ${result.txHash}`);
            } catch (err: unknown) {
              onAction(
                "token_transfer",
                `error: ${err instanceof Error ? err.message : "unknown"}`,
      ...
      {lastTx && (
        <div className="mt-3 p-3 rounded-lg bg-green-500/10 border border-green-500/20">
          ...
          <div className="text-[11px] text-white/60 space-y-0.5">
            <div>
              {lastTx.from} → {lastTx.to}: {lastTx.amount} $CULT
            </div>
            ...
            .map((msg) => (
              ...
              >
                <div className="flex items-center gap-2 mb-1 flex-wrap">
                  ...
                  <span
                    className={`text-xs font-medium ${cultNameColor(
                      msg.fromCultName,
                    )}`}
                  >
                    {msg.fromCultName}
      ...
      {loading && !data ? (
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <TokenTransferPanel cults={data.cults} onAction={handleAction} />
          </div>
        </div>
      ) : (
