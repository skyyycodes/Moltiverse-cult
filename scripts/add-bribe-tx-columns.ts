...
async function main() {
  console.log("Adding transfer_tx_hash and transfer_status columns to bribe_offers...");
  ...
  const { data, error } = await (client.database as any).rpc("exec_sql", { query: sql });
  if (error) {
    ...
    if (testError) {
      console.log("Columns don't exist yet. Please run this SQL manually in your InsForge dashboard:");
      console.log("");
      ...
      console.log("");
      console.log("Or the columns will be created automatically when the first bribe accept happens (with NULL values).");
    } else {
